...
export const CustomSelect = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      const isOutsideModal = modalContainerRef.current && !modalContainerRef.current.contains(event.target as Node);
  ...
  const renderNormalDropdown = () => (
    <motion.div
      className={`relative h-[102px] ${
        isOpen ? 'shadow-custom-xl w-[316px]' : 'w-full pt-8'
      }`}
    ...
    >
      ...
      <div
        ...
        className={`relative text-[16px] font-bold text-[#4f4f4f] outline-none border-b bg-transparent cursor-pointer flex justify-end ${
          isOpen
            ? 'w-[316px] h-14 px-4 border-transparent'
      ...
      >
        <div className='flex justify-center items-center w-6 h-6'>
          <IconCustom
            ...
            className='w-2.5 h-1.5 fill-none text-[#373f4c]'
          />
        </div>
      </div>
      {isOpen && !isOpening && (
        <div
          ref={dropdownRef}
          className='bg-white max-h-[200px] custom-scrollbar overflow-y-auto w-full max-w-[316px] absolute top-[60px] left-0 z-40'
        >
          {renderOptionsView()}
        </div>
      )}
        ...
        >
          ...
          >
            <div className='p-4 border-b border-transparent bg-transparent flex items-center justify-between'>
              <div className='text-[16px] font-bold text-[#3486fe]'>{label}</div>
              <div className='flex justify-center items-center w-6 h-6'>
                <IconCustom
                  name='arrow-down-select'
                  className='w-2.5 h-1.5 fill-none text-[#3486fe]'
